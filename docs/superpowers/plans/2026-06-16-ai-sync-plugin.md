...
**Architecture:** Чистое ядро синхронизации (`sync-core`, `track-extractor`, `sync-graph`) тестируется unit-тестами через vm-loader (как в родительском плагине). ffmpeg-извлечение огибающих и тонкий ExtendScript-host изолированы за `bridge-premiere`. UI-панель (CEP) валидируется вживую через `cep-debug.mjs` на порту 8099. Вся математика — в домене времени (секунды → тики), что делает её fps-агностичной.
...
| `CSXS/manifest.xml` + `CSXS/.debug` | манифест расширения + debug-порт 8099 |
...
- [ ] **Step 3: Создать `CSXS/.debug` (порт 8099)**
...
Run: `CEP_DEBUG_PORT=8099 node tools/cep-debug.mjs targets`
Expected: строка с `ИИ: синхронизация` и `ws://localhost:8099/...`

- [ ] **Step 8: Проверить загрузку модулей вживую**

Run: `CEP_DEBUG_PORT=8099 node tools/cep-debug.mjs eval "[!!window.SyncCore, !!window.AudioEnvelope, window.AudioEnvelope.hasNode()]"`
...
git commit -m "feat(cep): оболочка плагина — манифест, debug-порт 8099, панель-заглушка"
```
...
Run: `CEP_DEBUG_PORT=8099 node tools/cep-debug.mjs reload`
затем:
Run: `CEP_DEBUG_PORT=8099 node tools/cep-debug.mjs host '$._SYNC_.getTimelineSnapshot()'`
Expected: JSON с `"ok":true` и `sequenceName`

- [ ] **Step 5: Live-проверка moveClip + backup (безопасно, с откатом)**

Run: `CEP_DEBUG_PORT=8099 node tools/cep-debug.mjs host '$._SYNC_.backupActiveSequence()'`
...
Run: `CEP_DEBUG_PORT=8099 node tools/cep-debug.mjs host '$._SYNC_.moveClip("{\"nodeId\":\"<NODEID>\",\"deltaTicks\":10160640000}")'`
...
- [ ] **Step 1: Написать `README.md`** — назначение, установка (симлинк в CEP/extensions, включить PlayerDebugMode), требования (ffmpeg в PATH/`C:\ffmpeg\bin`), порт отладки 8099, `npm test`, краткий цикл «Анализ → превью → Применить → Откат».
...
- **§4.1 оболочка** → Task 8 (манифест, .debug 8099, csinterface, cep-debug). ✓
